<template>
  <div>
    ...
    <div v-if="!searched">
      <v-card dark>
        <v-card-title data-tests="no-city">No city searched. Go back to Search page and select a
          city.</v-card-title>
      </v-card>
    </div>
    <div v-if="jsonResponseBool && searched">
      <div v-if="hasEvents">
        <v-card>
          <v-card-title data-test="city-selected">Events: {{ city }}, {{ state }}</v-card-title>
        </v-card>
        <v-card
          class="ma-4"
          dark
        ...
        >
          ...
          <v-card-subtitle class="text-left">Event Type: {{ event.classifications[0].segment.name
            }}<span v-if="event.classifications[0].genre">, {{ event.classifications[0].genre.name }}</span></v-card-subtitle>
          <v-card-text>
